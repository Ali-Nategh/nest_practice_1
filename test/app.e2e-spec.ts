...
describe('App e2e', () => {
  ...
  describe('Auth', () => {
    ...
    describe('Signin', () => {
      ...
      it('should signin', () => {
        return pactum.spec().post('/auth/signin')
          .withBody(dto).expectStatus(200).stores('userAccessToken', 'access_token')
      })
    });
  });
    ...
    describe('Get User', () => {
      it('should throw if access token not provided', () => {
        return pactum.spec().get('/users/me')
          .withHeaders({ Authorization: '' }).expectStatus(401)
      })
      it('should throw if access token not valid', () => {
        return pactum.spec().get('/users/me')
          .withHeaders({ Authorization: 'Bearer $S{userAccessToken}test' }).expectStatus(401)
      })
      it('should throw if no header provided', () => {
        return pactum.spec().get('/users/me').expectStatus(401)
      })
      it('should get current user', () => {
        return pactum.spec().get('/users/me')
          .withHeaders({ Authorization: 'Bearer $S{userAccessToken}' }).expectStatus(200)
      })
    });
  ...
  describe('Bookmarks', () => {
    ...
    describe('Edit Bookmark By ID', () => {
      const dto: EditBookmarkDto = {
        title: 'Change The Title Boi',
        description: 'Damn What A Cool BOOOKMAAAARK'
      }
      it("should edit bookmark", () => {
        return pactum.spec().patch('/bookmarks/{id}').withPathParams('id', '$S{bookmarkID}')
          .withBody(dto).withHeaders({ Authorization: 'Bearer $S{userAccessToken}' })
